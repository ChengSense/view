(function () {
  ...
  function eachNode(newNode, node, child) {
    var comment = document.createComment("each:" + $path);
  ...
  function whenNode(newNode, node, child, content, scopes, attributes) {
    if (new RegExp($whea).test(child.clas.nodeValue)) {
      var comment = document.createComment("when:" + $path);
      node.appendChild(comment);
...
(function () {
  ...
  function observe(target, callSet, callGet) {
    ...
    function define(object, prop, path, oldValue) {
      ...
      Object.defineProperty(object, prop, {
        get() {
          mq.publish("get", target, [path]);
          return value;
        },
        set(val) {
          var oldValue = value;
          watcher(value = val, path, oldValue);
          if (setable) mq.publish("set", target, [path]);
        }
      });
    }
    ...
    mq.subscribe("set", target, callSet);
    mq.subscribe("get", target, callGet);
  ...
  class Mess extends Map {
    publish(event, scope, data) {
      const cache = this.get(event);
      if (cache) {
        cache.data.push(data);
      } else {
        let map = new Map();
        let data = { data: [data], queue: map };
        this.set(event, data);
      }
      this.notify(cache, scope);
    }
    ...
    notify(cache, scope) {
      if (cache) {
        while (cache.data.length) {
          ...
          const cals = cache.queue.get(scope);
          cals.forEach(function (call) {
        ...
        this.forEach(function (cache) {
          while (cache.data.length) {
            const data = cache.data.shift();
            cache.queue.forEach(function (cals) {
              cals.forEach(function (call) {
                call(data[0], data[1], data[2]);
              })
            });
    ...
    subscribe(event, scope, call) {
      ...
      if (cache) {
        ...
        if (cals) {
          ...
          cache.queue.set(scope, [call]);
        }
      } else {
        let map = new Map();
        map.set(scope, [call]);
        let data = { data: [], queue: map };
        this.set(event, data);
      }
    }
  }
...
(function () {
  ...
  function Router(app, params) {
    ...
    let listener = ["hashchange", "load", "popstate"];
    listener.forEach(handle => {
      window.addEventListener(handle, action, false);
    });

(function () {
  ...
  function View(app) {
    ...
    var attributes = [];
    var node = initCompiler(init(slice(query(app.view))))[0];
    ...
    resolver["view"](app.view, node, app.model, content, attributes);
  }
  ...
  var resolver = {
    view: function (dom, node, scope, content, attributes) {
      try {
        var doc = document.createDocumentFragment();
        compiler(doc, scope, slice(node.children), content, attributes);
        query(dom)[0].reappend(doc);
      } catch (e) {
        console.log(e);
      }
    },
  ...
  function commom(node, scope, clas, content, attributes) {
    ...
    if (new RegExp($express).test(node.nodeValue)) {
...
(function () {
  ...
  function observe(target, callSet, callGet) {
    ...
    function define(object, prop, path, oldValue) {
      ...
      Object.defineProperty(object, prop, {
        ...
        set(val) {
          ...
          if (setable) mq.publish("set", [path]);
        }
      });
    }
    ...
    mq.subscribe("set", callSet);
    mq.subscribe("get", callGet);
...
(function () {
  ...
  function Router(app, params) {
    ...
    window.addEventListener("hashchange", function () {
      ...
      app.model.router = router.router;
    }, false);
  }
